...
object RecursionPatternMatchingExercise {
  ...
  def checkValuesIncrease(seq: Seq[Int]): Boolean = {
    ...
    seq match{
      ...
      case x if seq.size <= 1 => true
      case _ =>  seq.head < seq.tail.head && checkValuesIncrease(seq.tail)
    }
  }
  ...
  def  groupConsecutive[T](in: List[T]): List[List[T]] = {
    println("LISTA: " + in.toString())
    val a = List()
    println("A: " + a.toString())
    in match {
      case x if in.length <= 1 => a :+ in.head
      case _ if (in.head == in.tail.head) =>  a :+ groupConsecutive(in.tail)
      case _=> groupConsecutive(in.tail)
    }
    a
  }
